...
with open("challenge-09-12-2019.txt", "r") as f:
  puzzle_input = [x.strip() for x in f]

def part1(input):
  
  program_data = [int(x) for x in input[0].split(',')]
  comp1 = IntCodeComputer(program_data.copy(), inptArr = [1])
  return(comp1.compute())

def part2(input):
  
  program_data = [int(x) for x in input[0].split(',')]
  comp1 = IntCodeComputer(program_data.copy(), inptArr = [2])
  return(comp1.compute())
